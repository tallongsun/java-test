...
/**
 * 说明：
 * 1.本地spark是1.6.1，依赖于scala2.10，因此依赖的spark的jar包为spark-core_2.10-1.6.1.jar
 * 2.该spark流程序跑起来后，可以通过localhost:4040可视化查看job运行情况
 *
 */
public class SparkStreamingKafkaLogAnalyzer {
	...
	public static void main(String[] args) {
		...
		SparkConf conf = new SparkConf().setAppName(appName).setMaster("local[*]").set("spark.executor.memory", "1g");
		...
		JavaPairReceiverInputDStream<String, String> logDataDStream = KafkaUtils.createStream(jssc, zkQuorum, group,
				topicMap);

		LOGGER.debug("Received DStream connecting to zookeeper " + zkQuorum + " group " + group + " topics" + topicMap);
		LOGGER.debug("logDataDStream: " + logDataDStream);

		JavaDStream<ApacheAccessLog> accessLogDStream = logDataDStream
				.map(new Function<Tuple2<String, String>, ApacheAccessLog>() {
					private static final long serialVersionUID = -5400998295385709178L;

					public ApacheAccessLog call(Tuple2<String, String> message) {
						LOGGER.debug("receive msg : "+message._1);
		...
		windowDStream.foreachRDD(new Function<JavaRDD<ApacheAccessLog>, Void>() {
			private static final long serialVersionUID = -4348992534211552992L;

			@Override
			public Void call(JavaRDD<ApacheAccessLog> accessLogs) {
				LOGGER.debug("Aceess Log count:"+accessLogs.count());
		...
		LOGGER.debug("sparkstream start..");
	}
}
